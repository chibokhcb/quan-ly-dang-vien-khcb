...
export class DataRepository {
  ...
  static getPartyMembers(): PartyMember[] {
    let list = loadStoredData<PartyMember[]>(STORAGE_KEYS.MEMBERS, INITIAL_PARTY_MEMBERS);
    
    // Auto-sync any missing party members from INITIAL_PARTY_MEMBERS
    const existingEmails = new Set(list.map((m) => m.workEmail?.toLowerCase()).filter(Boolean));
    let hasNew = false;
    INITIAL_PARTY_MEMBERS.forEach((initMember) => {
      if (initMember.workEmail && !existingEmails.has(initMember.workEmail.toLowerCase())) {
        list.push(initMember);
        if (initMember.workEmail) existingEmails.add(initMember.workEmail.toLowerCase());
        hasNew = true;
      }
    });
