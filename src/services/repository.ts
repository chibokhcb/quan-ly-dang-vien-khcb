...
import { fetchCollection, fetchDoc, setItem, setDocData, removeItem, setCollectionBulk, isFirestoreEnabled } from '../lib/firestoreSync';
...
// Firestore collection name mapping (used only when isFirestoreEnabled())
const COLLECTIONS = {
    MEMBERS: 'partyMembers',
    TRIPS: 'foreignTrips',
    CANDIDATES: 'developmentCandidates',
    ADMISSIONS: 'admissionDossiers',
    OFFICIALIZATIONS: 'officializationDossiers',
    CHANGE_REQUESTS: 'memberChangeRequests',
...
export class DataRepository {
    // --- FIRESTORE HYDRATION (real-data mode only) ---
  static async hydrateFromFirestore(): Promise<void> {
        if (!isFirestoreEnabled()) return;
        try {
                const collectionsToSync: { key: string; storageKey: string; defaultData: any[]; idField?: string }[] = [
                  { key: COLLECTIONS.MEMBERS, storageKey: STORAGE_KEYS.MEMBERS, defaultData: INITIAL_PARTY_MEMBERS },
                  { key: COLLECTIONS.TRIPS, storageKey: STORAGE_KEYS.TRIPS, defaultData: INITIAL_FOREIGN_TRIPS },
                  { key: COLLECTIONS.CANDIDATES, storageKey: STORAGE_KEYS.CANDIDATES, defaultData: INITIAL_DEVELOPMENT_CANDIDATES },
                  { key: COLLECTIONS.ADMISSIONS, storageKey: STORAGE_KEYS.ADMISSIONS, defaultData: INITIAL_ADMISSION_DOSSIERS },
                  { key: COLLECTIONS.OFFICIALIZATIONS, storageKey: STORAGE_KEYS.OFFICIALIZATIONS, defaultData: INITIAL_OFFICIALIZATION_DOSSIERS },
                  { key: COLLECTIONS.CHANGE_REQUESTS, storageKey: STORAGE_KEYS.CHANGE_REQUESTS, defaultData: INITIAL_CHANGE_REQUESTS },
                  { key: COLLECTIONS.POLICY_VERSIONS, storageKey: STORAGE_KEYS.POLICY_VERSIONS, defaultData: INITIAL_POLICY_VERSIONS },
                  { key: COLLECTIONS.USERS, storageKey: STORAGE_KEYS.USERS, defaultData: INITIAL_USER_ACCOUNTS, idField: 'uid' },
                  { key: COLLECTIONS.AUDIT_LOGS, storageKey: STORAGE_KEYS.AUDIT_LOGS, defaultData: INITIAL_AUDIT_LOGS },
                        ];

          for (const c of collectionsToSync) {
                    const remote = await fetchCollection<any>(c.key);
                    if (remote && remote.length > 0) {
                                saveStoredData(c.storageKey, remote);
                    } else {
                                const local = loadStoredData<any[]>(c.storageKey, c.defaultData);
                                if (local && local.length > 0) {
                                              await setCollectionBulk(c.key, local, c.idField || 'id');
                                }
                    }
          }

          const remoteSettings = await fetchDoc<AppSettings>('appSettings', 'global');
                if (remoteSettings) {
                          saveStoredData(STORAGE_KEYS.SETTINGS, remoteSettings);
                } else {
                          const localSettings = this.getSettings();
                          setDocData('appSettings', 'global', localSettings);
                }
        } catch (err) {
                console.warn('[DataRepository] hydrateFromFirestore failed:', err);
        }
  }
  ...
  static savePartyMember(member: Partial<PartyMember>, actorEmail: string): PartyMember {
      ...
      saveStoredData(STORAGE_KEYS.MEMBERS, members);
        setItem(COLLECTIONS.MEMBERS, savedMember.id, savedMember);
        this.addAuditLog(
                actorEmail,
  ...
  static adjustPartyMemberName(
        ...
        if (index >= 0) {
          ...
          saveStoredData(STORAGE_KEYS.MEMBERS, members);
                setItem(COLLECTIONS.MEMBERS, id, members[index]);
                this.addAuditLog(
                          actorEmail,
                          'ADJUST_MEMBER_NAME',
                          'PARTY_MEMBER',
  ...
  static deletePartyMember(
      ...
      if (index >= 0) {
          ...
          if (isPermanent) {
                    ...
                    saveStoredData(STORAGE_KEYS.MEMBERS, members);
                    removeItem(COLLECTIONS.MEMBERS, id);
                    this.addAuditLog(
                                actorEmail,
  ...
  static saveForeignTrip(trip: Partial<ForeignTrip>, actorEmail: string): ForeignTrip {
      ...
      if (index >= 0) {
        ...
        setItem(COLLECTIONS.TRIPS, saved.id, saved);
  ...
  static deleteForeignTrip(id: string, actorEmail: string): boolean {
        ...
        if (index >= 0) {
                ...
                saveStoredData(STORAGE_KEYS.TRIPS, trips);
                removeItem(COLLECTIONS.TRIPS, id);
                this.addAuditLog(
                          actorEmail,
                          'DELETE_FOREIGN_TRIP',
                          'FOREIGN_TRIP',
  ...
  static importForeignTrips(tripsData: Partial<ForeignTrip>[], actorEmail: string): number {
      ...
      saveStoredData(STORAGE_KEYS.TRIPS, existing);
        setCollectionBulk(COLLECTIONS.TRIPS, existing, 'id');
  ...
  static saveDevelopmentCandidate(cand: Partial<DevelopmentCandidate>, actorEmail: string): DevelopmentCandidate {
      ...
      saveStoredData(STORAGE_KEYS.CANDIDATES, candidates);
        setItem(COLLECTIONS.CANDIDATES, saved.id, saved);
  ...
  static deleteDevelopmentCandidate(id: string, actorEmail: string): boolean {
        ...
        if (index >= 0) {
                ...
                saveStoredData(STORAGE_KEYS.CANDIDATES, candidates);
                removeItem(COLLECTIONS.CANDIDATES, id);
                this.addAuditLog(
                          actorEmail,
                          'DELETE_CANDIDATE',
  ...
  static saveAdmissionDossier(dossier: AdmissionDossier, actorEmail: string): AdmissionDossier {
      ...
      if (index >= 0) {
        ...
        setItem(COLLECTIONS.ADMISSIONS, dossier.id, dossier);
  ...
  static deleteAdmissionDossier(id: string, actorEmail: string): boolean {
        ...
        if (index >= 0) {
                ...
                saveStoredData(STORAGE_KEYS.ADMISSIONS, dossiers);
                removeItem(COLLECTIONS.ADMISSIONS, id);
  ...
  static toggleArchiveAdmissionDossier(id: string, archive: boolean, actorEmail: string): boolean {
        ...
        if (index >= 0) {
                ...
                saveStoredData(STORAGE_KEYS.ADMISSIONS, dossiers);
                setItem(COLLECTIONS.ADMISSIONS, id, dossiers[index]);
  ...
  static saveOfficializationDossier(dossier: OfficializationDossier, actorEmail: string): OfficializationDossier {
      ...
      if (index >= 0) {
        ...
        setItem(COLLECTIONS.OFFICIALIZATIONS, dossier.id, dossier);
  ...
  static deleteOfficializationDossier(id: string, actorEmail: string): boolean {
        ...
        if (index >= 0) {
                const removed = dossiers.splice(index, 1)[0];
                saveStoredData(STORAGE_KEYS.OFFICIALIZATIONS, dossiers);
                removeItem(COLLECTIONS.OFFICIALIZATIONS, id);
  ...
  static toggleArchiveOfficializationDossier(id: string, archive: boolean, actorEmail: string): boolean {
        ...
        if (index >= 0) {
                ...
                dossiers[index].updatedAt = new Date().toISOString();
                saveStoredData(STORAGE_KEYS.OFFICIALIZATIONS, dossiers);
                setItem(COLLECTIONS.OFFICIALIZATIONS, id, dossiers[index]);
  ...
  static saveMemberChangeRequest(req: MemberChangeRequest, actorEmail: string): MemberChangeRequest {
        ...
        setItem(COLLECTIONS.CHANGE_REQUESTS, req.id, req);
  ...
  static savePolicyVersion(pv: PolicyVersion, actorEmail: string): PolicyVersion {
        ...
        setItem(COLLECTIONS.POLICY_VERSIONS, pv.id, pv);
  ...
  static saveUser(user: UserAccount, actorEmail: string): UserAccount {
        ...
        setItem(COLLECTIONS.USERS, user.uid, user);
  ...
  static saveUserAccount(user: Partial<UserAccount>): UserAccount {
        ...
        saveStoredData(STORAGE_KEYS.USERS, users);
        setItem(COLLECTIONS.USERS, saved.uid, saved);
        return saved;
  }
  ...
  static deleteUser(uid: string, actorEmail: string): boolean {
        ...
        if (index >= 0) {
                ...
                saveStoredData(STORAGE_KEYS.USERS, users);
                removeItem(COLLECTIONS.USERS, uid);
  ...
  static addAuditLog(actorEmail: string, action: string, resourceType: string, resourceId: string, maskedSummary: string): void {
      ...
      setItem(COLLECTIONS.AUDIT_LOGS, newLog.id, newLog);
  }
  ...
  static saveSettings(settings: AppSettings, actorEmail: string): void {
        ...
        setDocData('appSettings', 'global', settings);
